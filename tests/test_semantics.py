...
Die Schichtlogik wird mit Dummy-Vektoren ueber eine injizierte Embedding-Funktion
geprueft, ohne Modell-Lauf. Themen-Marker im Notiztext steuern, welche Knoten
sich gleichen, so sind die Aehnlichkeiten vollstaendig kontrolliert.
...
- Schichtlogik, eine latente Verknuepfung ist ein hochaehnliches Paar ohne
  direkten Wikilink, ein direkt verlinktes Paar wird trotz Aehnlichkeit nicht
  gemeldet, die Nachbarliste haelt top-k ein.
"""
...
class TestSchichtlogik:
    ...
    def test_same_theme_unlinked_is_latent(self, tmp_path: Path):
        result = _result(tmp_path, threshold=0.9)
    ...
    def test_latent_pair_carries_cosine_and_distance(self, tmp_path: Path):
        ...
        # Alpha und Beta haengen in getrennten Komponenten, Distanz unbestimmt.
        assert ab["link_distance"] is None

    def test_directly_linked_pair_not_latent(self, tmp_path: Path):
        result = _result(tmp_path, threshold=0.9)
        pairs = {(r["a"], r["b"]) for r in result["latent_links"]}
        # Gamma und Delta teilen das Thema, sind aber direkt verlinkt.
...
class TestDeterminism:
    def test_artifact_byte_identical_over_two_runs(self, tmp_path: Path):
        result1 = _result(tmp_path, top_k=4, threshold=0.9)
        result2 = _result(tmp_path, top_k=4, threshold=0.9)
